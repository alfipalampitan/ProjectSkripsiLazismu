...
const form = useForm({
    user_name: '',
    amount: '', // Tetap menyimpan angka murni (contoh: 100000)
    kategori: '',
    nomor_hp: '',
    email: '',
    program_id: '',
...
// State lokal untuk menampilkan format titik di UI
...
const formatRupiah = (value) => {
    if (!value) return '';
    const numberString = value.toString().replace(/[^0-8\d]/g, ''); // Ambil angka saja
    return numberString.replace(/\B(?=(\d{3})+(?!\d))/g, ".");
};

// Fungsi yang dipanggil setiap kali user mengetik di input nominal
const handleAmountInput = (event) => {
    const rawValue = event.target.value.replace(/\./g, ''); // Buang semua titik
    
    // Pastikan hanya angka yang masuk
    if (!isNaN(rawValue)) {
        form.amount = rawValue; // Simpan angka bersih ke object form Inertia
        displayAmount.value = formatRupiah(rawValue); // Tampilkan format titik ke user
    }
};
...
const submit = () => {
    form.post(route('donasi-tunai.store'), {
        ...
        onSuccess: () => {
            form.reset();
            displayAmount.value = ''; // Reset tampilan nominal
            Swal.fire({
                icon: 'success',
                title: 'Alhamdulillah!',
    ...
    <StaffLayout>
        <div class="max-w-4xl mx-auto py-8 px-4">
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-500 font-medium">Input manual untuk donasi yang diterima secara langsung (Cash/Offline).</p>
            </div>
            ...
            <form @submit.prevent="submit" class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="md:col-span-2 space-y-6">
                    <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-5">
                        ...
                        <!-- Nama Donatur -->
                        <div>
                            <label class="text-xs font-black uppercase text-gray-400 ml-2 tracking-widest">Nama Donatur</label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="text-xs font-black uppercase text-gray-400 ml-2 tracking-widest">Nominal Donasi (Rp)</label>
                                <!-- DIUBAH KE TYPE="TEXT", MENGGUNAKAN VALUE DAN @INPUT -->
                                <input :value="displayAmount" @input="handleAmountInput" inputmode="numeric" placeholder="Contoh: 100.000"
                                    ...
                                    required>
                                <div v-if="form.errors.amount" class="text-red-500 text-xs mt-1 ml-2">
                                    {{ form.errors.amount }}
                                </div>
                            </div>
                            <!-- Tampilan Kategori (ReadOnly) -->
                            <div>
                                <label class="text-xs font-black uppercase text-gray-400 ml-2">Kategori Terdeteksi</label>
                                <div class="w-full mt-1 px-6 py-4 rounded-2xl bg-gray-100 text-gray-500 font-bold border-2 border-dashed border-gray-200">
                        ...
                        <!-- Keterangan -->
                        <div>
                            <label class="text-xs font-black uppercase text-gray-400 ml-2 tracking-widest">Keterangan / Pesan</label>
                ...
                <div class="space-y-6">
                    <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-5">
                        <h3 class="text-sm font-black text-gray-800 uppercase tracking-tighter">Kontak Donatur (Opsional)</h3>

                        <div>
                            <label class="text-[10px] font-black uppercase text-gray-400 ml-2">WhatsApp / HP</label>
                            <input v-model="form.nomor_hp" type="tel" placeholder="0812..."
                                class="w-full mt-1 px-5 py-3 rounded-xl bg-gray-50 border-none focus:ring-2 focus:ring-orange-500 text-sm">
                        </div>

                        <div>
                            <label class="text-[10px] font-black uppercase text-gray-400 ml-2">Email (Opsional)</label>
                    ...
                    <div class="space-y-3">
                        ...
                        <button type="button" @click="form.reset(); displayAmount = '';"
