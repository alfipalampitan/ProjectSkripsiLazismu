    ...
    <div class="min-h-screen bg-[#F3F4F6] flex relative overflow-hidden">
        ...
        <transition name="fade">
            <div v-if="isMobileMenuOpen" @click="isMobileMenuOpen = false"
                class="fixed inset-0 bg-gray-900/50 z-[60] md:hidden backdrop-blur-sm transition-opacity"></div>
        </transition>
        ...
        <aside :class="[
            ...
            <div class="p-6 flex items-center justify-between border-b border-gray-50 h-20 md:h-24">
                <Link href="/" class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 bg-orange-500 rounded-xl flex items-center justify-center shadow-lg shadow-orange-200">
                    ...
                    <div v-if="isMobileMenuOpen" class="flex flex-col transition-all">
                        <span class="text-xl font-black text-gray-800 leading-none tracking-tight">LAZIS<span
                                class="text-orange-500">MU</span></span>
            ...
            <nav class="flex-grow p-4 space-y-3 overflow-y-auto custom-scrollbar">
                <div v-if="isMobileMenuOpen"
                    class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 ml-2">Menu Utama</div>

                <Link v-for="link in navLinks" :key="link.name" :href="link.url"
                    class="flex items-center gap-4 p-3.5 rounded-2xl transition-all duration-300 group" :class="[
                        isActive(link)
                    ...
                    <span v-if="isMobileMenuOpen" class="font-bold text-sm transition-all truncate">{{ link.name
                        }}</span>
                </Link>
            </nav>
            ...
            <div v-if="isMobileMenuOpen" class="p-6 border-t border-gray-50 transition-all">
                <div class="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                    ...
                    <a href="https://wa.me/6285753717680?text=Halo%20Admin%20Lazismu%2C%20saya%20butuh%20bantuan%20terkait%20program%20kebaikan."
                        target="_blank"
                        class="w-full bg-green-500 hover:bg-green-600 text-white py-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md">
        ...
        <div class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
            ...
            <header
                class="bg-white shadow-sm border-b border-gray-50 h-16 md:h-20 flex items-center justify-between px-4 md:px-10 z-50">
                <div class="flex items-center gap-4">
                    ...
                    <button @click="isMobileMenuOpen = !isMobileMenuOpen"
                        ...
                        <i
                            :class="['fa-solid transition-transform duration-500 text-lg', isMobileMenuOpen ? 'fa-bars-staggered' : 'fa-bars']"></i>
                    </button>
